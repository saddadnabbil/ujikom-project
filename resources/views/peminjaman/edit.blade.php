@extends('layouts.main', ['title' => 'Edit Peminjaman', 'page_heading' => 'Edit Data Peminjaman'])
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-content">
                    <div class="card-body">
                        <form action="{{ route('peminjaman.update', $peminjaman->id) }}" method="POST" class="form form-horizontal">
                            @csrf
                            @method('PUT')
                            <div class="form-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-8 form-group">
                                        <input type="text" id="id_pinjam" class="form-control @error('id_pinjam') is-invalid @enderror" 
                                            name="id_pinjam" value="{{ old('id_pinjam', $peminjaman->id_pinjam) }}" required>
                                        @error('id_pinjam')
                                    ...
                                    <div class="col-md-8 form-group">
                                        <input type="number" id="lama_pinjam" class="form-control @error('lama_pinjam') is-invalid @enderror" 
                                            name="lama_pinjam" value="{{ old('lama_pinjam', $peminjaman->lama_pinjam) }}" required>
                                        @error('lama_pinjam')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                    ...
                                    <div class="col-md-8 form-group">
                                        <input type="number" id="nominal_denda" class="form-control @error('nominal_denda') is-invalid @enderror" 
                                            name="nominal_denda" value="{{ old('nominal_denda', $peminjaman->nominal_denda) }}" required>
                                        @error('nominal_denda')
                                            <div class="invalid-feedback">
                                                {{ $message }}
                                            </div>
                                        ...
                                        <select name="id_anggota" id="id_anggota" class="form-select @error('id_anggota') is-invalid @enderror" required>
                                            ...
                                            @foreach ($anggotas as $anggota)
                                                <option value="{{ $anggota->id }}" 
                                                    {{ old('id_anggota', $peminjaman->id_anggota) == $anggota->id ? 'selected' : '' }}>
                                                    {{ $anggota->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="id_user" id="id_user" class="form-select @error('id_user') is-invalid @enderror" required>
                                            ...
                                            @foreach ($users as $user)
                                                <option value="{{ $user->id }}" 
                                                    {{ old('id_user', $peminjaman->id_user) == $user->id ? 'selected' : '' }}>
                                                    {{ $user->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="id_denda" id="id_denda" class="form-select @error('id_denda') is-invalid @enderror" required>
                                            ...
                                            @foreach ($dendas as $denda)
                                                <option value="{{ $denda->id }}" 
                                                    {{ old('id_denda', $peminjaman->id_denda) == $denda->id ? 'selected' : '' }}>
